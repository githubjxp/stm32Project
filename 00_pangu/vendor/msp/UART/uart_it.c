...
{
    HAL_UART_AbortReceive(uartCfg->uart); // 取消接收，清除DMA状态，否则会导致新接收无法触发
    if (uartCfg->uart->hdmarx != NULL) {
        HAL_UARTEx_ReceiveToIdle_DMA(uartCfg->uart, uartBuff->buff + uartBuff->pos, uartBuff->size - uartBuff->pos);
    } else {
        HAL_UARTEx_ReceiveToIdle_IT(uartCfg->uart, uartBuff->buff + uartBuff->pos, uartBuff->size - uartBuff->pos);
    }
}
...
{
    PRINT_FOR_TEST("[START]buff0 pos: %d, buff1 pos: %d, state: %d",
    ...
    // Switch buff with warn: current buff is full and other buff is not empty
        ...
        uartReceiveToIdle(uartCfg, &uartCfg->rxBuff0);
        uartCfg->rxBuffState = UART_BUFF_STATE_BUFF0_RECEIVING;
        PRINT_WARN("unexpect error, force switch to buff0");
    }

    PRINT_FOR_TEST("[END]buff0 pos: %d, buff1 pos: %d, state: %d",
        uartCfg->rxBuff0.pos, uartCfg->rxBuff1.pos, uartCfg->rxBuffState);
}
...
{
    PRINT_FOR_TEST("HAL_UART_RxCpltCallback called");
    UartConfig_t *comm= getUartConfigByFuncID(UART_FUNC_ID_PC_COMMUNICATION);
    if (huart->Instance == comm->uart->Instance) {
        uartBuffSwitch(comm);    
        xTaskNotifyFromISR(g_cmdTaskHandle, 1, eSetValueWithOverwrite, NULL);
    }
}
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t size)
{
    ...
    if ((huart->Instance == comm->uart->Instance)) {
        ...
        // xTaskNotifyFromISR(g_cmdTaskHandle, 1, eSetValueWithOverwrite, NULL); // Notify the command task with size
    }
}
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
    ...
    if (huart->Instance == comm->uart->Instance) {
        uartBuffSwitch(comm);
        vTaskNotifyGiveFromISR(g_cmdTaskHandle, 0);    
        // xTaskNotifyFromISR(g_cmdTaskHandle, 1, eSetValueWithOverwrite, NULL);
    }
}
